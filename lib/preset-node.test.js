...
`);
	...
	test('syntax undefined', () => {
		...
		try {
			...
			expect(error).toMatchInlineSnapshot(`
[Error: @backtrack/preset-node: "syntax" must be explicitly set to one of the following: node, babel, typescript, flow
found in path: <PROJECT_ROOT>/lib]
`);
		}
	});
...
`);
	...
	test('syntax not supported', () => {
		...
		try {
			...
			expect(error).toMatchInlineSnapshot(`
[Error: @backtrack/preset-node: "syntax: python" must be one of the following: node, babel, typescript, flow
found in path: <PROJECT_ROOT>/lib]
`);
		}
	});
